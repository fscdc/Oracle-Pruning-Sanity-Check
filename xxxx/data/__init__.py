...
class Data(object):
    def __init__(self, args):
        ...
        kwargs = {}
        if args.debug and args.dataset in [
            "imagenet",
            "imagenet_subset_100",
            "imagenet_subset_200",
            "clip_imagenet",
            ...
            if not os.path.exists(train_folder_path):
                ...
                [
                    shutil.copytree(
                        f"{args.data_path}/{args.dataset_dir}/train/{d}",
                        f"{train_folder_path}/{d}",
                    )
                    for d in dirs
                ]
            # Copy val
            if not os.path.exists(val_folder_path):
                ...
                [
                    shutil.copytree(
                        f"{args.data_path}/{args.dataset_dir}/val/{d}",
                        f"{val_folder_path}/{d}",
                    )
                    for d in dirs
                ]
        ...
        train_raw_set, _ = dataset.get_raw_dataset(dataset_path, **kwargs)

        train_sampler = None
        train_subset_sampler = None
        if args.distributed:
            train_sampler = torch.utils.data.distributed.DistributedSampler(train_set)
            train_subset_sampler = torch.utils.data.distributed.DistributedSampler(
                train_raw_set
            )
        ...
        self.train_loader_raw = DataLoader(
            train_raw_set,
            batch_size=args.batch_size,
            num_workers=args.workers,
            shuffle=(train_subset_sampler is None),
            pin_memory=True,
        ...
        if hasattr(args, "batch_size_prune"):
            ...
            self.train_loader_prune = DataLoader(
                train_set,
