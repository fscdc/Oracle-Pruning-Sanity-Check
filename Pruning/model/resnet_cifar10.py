...
number of learnable_layers and parameters:

name      | learnable_layers | params
ResNet20  |    20  | 0.27M
ResNet32  |    32  | 0.46M
ResNet44  |    44  | 0.66M
ResNet56  |    56  | 0.85M
...
class ZeroPaddingX2(nn.Identity):
    def forward(self, x):
        num_channels = x.shape[1]
        y = F.pad(
            x[:, :, ::2, ::2],  # stride = 2, per Kaiming's ResNet (CVPR'16) paper
            (0, 0, 0, 0, num_channels // 2, num_channels // 2),
            "constant",
            0,
        )
        return y
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option="A", conv_type="default"):
        ...
        Conv2d = (
            Conv2D_WN if conv_type == "wn" else nn.Conv2d
        )  # @mst: weight normalization
        self.conv1 = Conv2d(
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = Conv2d(
            3, 16 * width_mul, kernel_size=3, stride=1, padding=1, bias=False
        )
        self.bn1 = nn.BatchNorm2d(16 * width_mul)
        self.layer1 = self._make_layer(
            block, 16 * width_mul, num_blocks[0], stride=1, option=option
        )
        self.layer2 = self._make_layer(
            block, 32 * width_mul, num_blocks[1], stride=2, option=option
        )
        self.layer3 = self._make_layer(
            block, 64 * width_mul, num_blocks[2], stride=2, option=option
        )
...
def resnet20(num_classes=10, **kwargs):
    return ResNet(
        BasicBlock, [3, 3, 3], num_classes=num_classes, conv_type=kwargs["conv_type"]
    )


def resnet32(num_classes=10, **kwargs):
    return ResNet(
        BasicBlock, [5, 5, 5], num_classes=num_classes, conv_type=kwargs["conv_type"]
    )


def resnet44(num_classes=10, **kwargs):
    return ResNet(
        BasicBlock, [7, 7, 7], num_classes=num_classes, conv_type=kwargs["conv_type"]
    )


def resnet56(num_classes=10, **kwargs):
    return ResNet(
        BasicBlock, [9, 9, 9], num_classes=num_classes, conv_type=kwargs["conv_type"]
    )


def resnet110(num_classes=10, **kwargs):
    return ResNet(
        BasicBlock, [18, 18, 18], num_classes=num_classes, conv_type=kwargs["conv_type"]
    )
...
# ------------------------
# Below are the models with 1x1 conv as downsample learnable_layers
