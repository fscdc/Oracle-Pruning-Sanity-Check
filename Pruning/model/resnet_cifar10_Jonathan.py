...
# --------------------------------------------------
# @mingsun-tse: This file refers to https://github.com/facebookresearch/open_lth/blob/main/models/cifar_resnet.py
# We adapt the code under the MIT license.
# --------------------------------------------------
...
class Model(nn.Module):
    ...
    def __init__(self, plan, initializer=None, outputs=None):
        ...
        # Initialize.
        # self.apply(initializer) # @mingsun-tse: Use the default init, so commented
    ...
    # ------- @mingsun-tse: We do not need some of functions below, so commented
    ...
    @staticmethod
    def get_model_from_name(model_name, initializer=None, outputs=10):
        ...
        convolutional learnable_layers surrounded by a residual connection. Each layer in the first segment
        ...
        N is the total number of learnable_layers in the network: 2 + 6D.
        The default value of W is 16 if it isn't provided.

        For example, ResNet-20 has 20 learnable_layers. Exclusing the first convolutional layer and the final
        linear layer, there are 18 convolutional learnable_layers in the blocks. That means there are nine
        ...
        print(f"resnet cifar10 arch plan: {plan}")
...
def resnet20(num_classes=10, **kwargs):
    return Model.get_model_from_name("cifar_resnet_20", outputs=num_classes)


def resnet56(num_classes=10, **kwargs):
    return Model.get_model_from_name("cifar_resnet_56", outputs=num_classes)
