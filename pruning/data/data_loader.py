...
class CelebA_multi_attr(data.Dataset):
    def __init__(self, img_dir, label_file, transform):
        self.img_list = [
            os.path.join(img_dir, i) for i in os.listdir(img_dir) if is_img(i)
        ]
...
class CelebA(data.Dataset):
    def __init__(self, img_dir, label_file, transform):
        self.img_list = [
            os.path.join(img_dir, i) for i in os.listdir(img_dir) if i.endswith(".npy")
        ]
        self.transform = transform
        if label_file.endswith(".npy"):
            ...
            for line in open(label_file):  # label file is txt
                if ".jpg" not in line:
                    continue
                img_name, *attr = line.strip().split()
                self.label[img_name] = int(
                    attr[2] == "1"
                )  # "Attractive" is at the third position of all attrs
...
class CelebA_npy(data.Dataset):
    def __init__(self, npy_dir, label_file, transform):
        self.npy_list = [
            os.path.join(npy_dir, i)
            for i in os.listdir(npy_dir)
            if i.endswith(".npy") and i != "batch.npy"
        ]
        self.transform = transform
        self.label = torch.from_numpy(
            np.load(label_file)
        ).long()  # label_file should be an npy
