    ...
    do
        ...
        do
            CUDA_VISIBLE_DEVICES=$i python main.py --no_ddp --no_scp \
                ...
                --pretrained_ckpt ./Experiments/scratch__lenet5_mini__kmnist__epochs30_lr0.01to0.001_SERVER227-20241027-133550/weights/ckpt_best.pth \
                --pruner oracle --prune_method oracle --prune_with_hooks \
                --stage_pr "*conv1*:$stage_pr" \
                --lr_ft 0:0.001,20:1e-4 --epochs 30 \
                --weight_decay 1e-4 \
                --batch_oracle $i \
                --experiment_name oracle__lenet5_mini__kmnist__prconv1${stage_pr//./}__epoch30_lr0.001to0.0001 > ./record/kmnist_conv1${stage_pr//./}_lr2_t$t\_batch$i.txt &
        done
        wait
    done
done
