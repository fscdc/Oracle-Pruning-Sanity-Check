from smilelogging.utils import (
    _weights_init,
    _weights_init_orthogonal,
    orthogonalize_weights,
    delta_orthogonalize_weights,
)
import torch
    ...
    def optimize(w, layer_name):
        ...
        for i in range(n_iter):
            ...
            if not isinstance(mask, type(None)):
                w_ = torch.mul(
                    w_, mask[layer_name].view_as(w_)
                )  # Not update the pruned params. For filter pruning, mask will be None
            w_ = torch.autograd.Variable(w_, requires_grad=True)
            optim = torch.optim.Adam([w_], lr)
            if i % 100 == 0:
                print(
                    '[%d/%d] approximate_isometry_optimize for layer "%s", loss %.6f'
                    % (i, n_iter, name, loss.item())
                )
...
def reinit_model(model, args, mask, print_fn):
    # ====== PyTorch (nearly) default init schemes:
    if args.reinit in [
        "default",
        "kaiming_normal",
    ]:  # Note this 'default' is NOT pytorch default init scheme! See 'pth_reset' below. Will be deprecated in the future
        model.apply(_weights_init)  # Completely reinit weights via 'kaiming_normal'
        if mask is not None:
            raise NotImplementedError
        print_fn(
            f"==> Reinit model: {args.reinit} ('kaiming_normal' for Conv/FC; 0 mean, 1 std for BN)"
        )
        ...
        print_fn(f"==> Reinit model: {args.reinit}")

    elif args.reinit in ["xavier_uniform"]:
        for name, module in model.named_modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                nn.init.xavier_uniform_(
                    module.weight, gain=nn.init.calculate_gain(args.activation)
                )
                # For unstructured pruning
                if mask is not None:
                    w = module.weight
    ...
    # ====== Exact isometry (orthogonalization) init schemes:
        ...
        model.apply(
            lambda m: _weights_init_orthogonal(
                m, act=args.activation, scale=args.reinit_scale
            )
        )  # Reinit weights via 'orthogonal_' from scratch
        if mask is not None:
            for name, module in model.named_modules():
                if isinstance(module, (nn.Conv2d, nn.Linear)):
                    w = module.weight
                    w.data.copy_(w.data * mask[name])
        print_fn(
            f"==> Reinit model: {args.reinit} ('orthogonal_' for Conv/FC; 0 mean, 1 std for BN)"
        )

    elif args.reinit in ["exact_isometry_based_on_existing"]:
        exact_isometry_based_on_existing_weights(
            model, act=args.activation, print=print_fn
        )  # Orthogonalize weights based on existing weights
        if mask is not None:
            raise NotImplementedError
        print_fn(
            f"==> Reinit model: {args.reinit} (orthogonalize Conv/FC weights based on existing weights)"
        )

    elif args.reinit in ["exact_isometry_based_on_existing_delta"]:
        if mask is not None:
            raise NotImplementedError
        exact_isometry_based_on_existing_weights_delta(
            model, act=args.activation, print=print_fn
        )
        print_fn(f"==> Reinit model: {args.reinit}")
    ...
    # ====== Approximate isometry init schemes:
        ...
        approximate_isometry_optimize(
            model, mask=mask, lr=args.lr_AI, n_iter=10000, print=print_fn
        )  # 10000 refers to the paper above; lr in the paper is 0.1, but not converged here
        print_fn(f"==> Reinit model: {args.reinit}")
    ...
    # ======
        ...
        from smilelogging.utils import AverageMeter, apply_mask
        ...
        for block_ix in range(len(blocks)):
            print_fn("=" * 30 + f" Block {block_ix} " + "=" * 30)
            ...
            for epoch in range(num_epochs):
                ...
                for i, (images, target) in enumerate(train_loader):
                    ...
                    if loss_scheme in ["v2"]:
                        ...
                        if i % 100 == 0:
                            print_fn(
                                f"[Data-dependent reinit]  Block {block_ix}  Epoch {epoch}  Step {i}  LossReg {loss_reg:.6f}"
                            )
                    ...
                    if i % 100 == 0:
                        print_fn(
                            f"[Data-dependent reinit]  Block {block_ix}  Epoch {epoch}  Step {i}  TrainLossAvg {train_loss.avg:.6f}  LR {lr}"
                        )
    ...
    if args.reinit.endswith("+AI"):
        ...
        print_fn(f"==> Reinit model: Applied approximate_isometry (AI)")
...
def orth_regularization_v6(w, pruned_wg, penalty_map):
    ...
    for x in pruned_wg:
        identity[x, x] = 0
    loss_map = (
        F.mse_loss(torch.matmul(w_, w_.t()), identity, reduction="none") * penalty_map
    )
    loss = loss_map.mean()
    return loss
